...
sayHelloButton.addEventListener('mouseover', editBtn)
...
sayHelloButton.addEventListener('mouseout', revertButtonBack)
...
/*
    ...
    Add a 3rd event listener to sayHelloButton and trigger the sayHello function when the button is
     clicked
*/
...
/* 
    Back in the ohMy function on Problem 5, r
    eplace the console log in the promise's callback with a for loop that loops over res.data. 

    On each iteration of the loop, create a new p element. 
    Set its textContent equal the string at the current index (i) and then append 
...
const attachQuery = () => {
    axios.get(`http://localhost:3000/query-test?button=${'dkddjf'}?button=${'dkdjjd'}`)
        .then((res) => {
            console.log(res.data)
